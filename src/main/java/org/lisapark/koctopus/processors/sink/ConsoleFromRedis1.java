 ...
 */
@Persistable
public class ConsoleFromRedis1 extends AbstractExternalSink {

    private static final String DEFAULT_NAME = "Console for Redis 1";
    ...
    private ConsoleFromRedis1(UUID id, String name, String description) {
    ...
    private ConsoleFromRedis1(UUID id, ConsoleFromRedis1 copyFromNode) {
        super(id, copyFromNode.getName(), copyFromNode.getDescription());
        input = copyFromNode.getInput().copyOf();
    }

    private ConsoleFromRedis1(ConsoleFromRedis1 copyFromNode) {
    ...
    @Override
    public ConsoleFromRedis1 newInstance() {
        return new ConsoleFromRedis1(Generators.timeBasedGenerator().generate(), this);
    }

    @Override
    public ConsoleFromRedis1 newInstance(Gnode gnode) {
        String uuid = gnode.getId() == null ? Generators.timeBasedGenerator().generate().toString() : gnode.getId();
        ConsoleFromRedis1 sink = newTemplate(UUID.fromString(uuid));
        GraphUtils.buildSink(sink, gnode);
        return sink;
    }

    @Override
    public ConsoleFromRedis1 copyOf() {
        return new ConsoleFromRedis1(this);
    }

    public ConsoleFromRedis1 newTemplate() {
        UUID sinkId = Generators.timeBasedGenerator().generate();
        return newTemplate(sinkId);
    }

    public ConsoleFromRedis1 newTemplate(UUID sinkId) {
        ConsoleFromRedis1 consoleSink = new ConsoleFromRedis1(sinkId, DEFAULT_NAME, DEFAULT_DESCRIPTION);
    ...
    public <T extends ExternalSink> CompiledExternalSink compile(T sink) throws ValidationException {
        return new CompiledConsole((ConsoleFromRedis1) sink);
    }
    ...
    static class CompiledConsole extends CompiledExternalSink {
        ...
        private final ConsoleFromRedis1 sink;

        /**
         *
         * @param sink
         */
        protected CompiledConsole(ConsoleFromRedis1 sink) {
            super(sink);
            this.sink = sink;
        }
